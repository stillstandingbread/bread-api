...
// Function to fetch circulating supply from Helius API
async function getCirculatingSupply() {
  try {
    ...
    if (response.data && response.data.result && response.data.result.value) {
      // Return circulating supply as a float number
      return parseFloat(response.data.result.value.uiAmountString);
    } else {
      console.error('Helius response unexpected format:', response.data);
      return null;
    }
  } catch (error) {
